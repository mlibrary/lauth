...
module Lauth
  module API
    ...
    class APP < Hanami::API
      ...
      # index groups
      get "/groups" do
        Yabeda.lauth.index_count.increment({resource: "groups"}, by: 1)
        authorized!

        repo = Lauth::API::Repositories::Group.new(BDD.rom)
        document = {}
        groups = []
        repo.index.each do |group|
          groups << group.resource_object
        end
      ...
      # create group
      post "/groups" do
        Yabeda.lauth.create_count.increment({resource: "groups"}, by: 1)
        authorized!

        repo = Lauth::API::Repositories::Group.new(BDD.rom)
        document = JSON.parse(env["rack.input"].gets)
        group = repo.create(document)
      ...
      # read group
      get "/groups/:id" do |id|
        Yabeda.lauth.read_count.increment({resource: "groups"}, by: 1)
        authorized!

        repo = Lauth::API::Repositories::Group.new(BDD.rom)
        group = repo.read(params[:id])
      ...
      # update group
      put "/groups/:id" do |id|
        Yabeda.lauth.update_count.increment({resource: "groups"}, by: 1)
        authorized!

        repo = Lauth::API::Repositories::Group.new(BDD.rom)
        document = JSON.parse(env["rack.input"].gets)
        group = repo.update(document)
      ...
      # delete group
      delete "/groups/:id" do |id|
        Yabeda.lauth.delete_count.increment({resource: "groups"}, by: 1)
        authorized!
